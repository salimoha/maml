...
class DataGenerator(object):
    ...
    def __init__(self, num_samples_per_class, batch_size, config={}):
        ...
        if FLAGS.datasource == 'KDD':
            ...
            self.img_size = config.get('img_size', (316172,40))
            self.dim_input = np.prod(self.img_size)
            self.dim_output = self.num_classes
            metatrain_folder = config.get('metatrain_folder', './data/KDD_data/train.csv')
            if FLAGS.test_set:
                metaval_folder = config.get('metaval_folder', './data/KDD_data/test.csv')
            else:
                metaval_folder = config.get('metaval_folder', './data/KDD_data/val.csv')
    ...
    def make_data_tensor(self, train=True):
        data = pd.read_csv("./data/KDD_data/train.csv")
        data_x = data.loc[:, data.columns != 'col41']
        data_x = data_x.apply(pd.to_numeric, errors='ignore')
        labels = data.loc[:,data.columns == 'col41']
        data_x = data_x.as_matrix()

        
        #data_x = tf.cast(data_x, tf.float64) 
        labels = labels.as_matrix()
        #print("data type:",type(data_x))
        # make queue for tensorflow to read from
        #filename_queue = tf.train.string_input_producer(tf.convert_to_tensor(data_x), shuffle=False)
        ...
        if FLAGS.datasource == 'miniimagenet':
            ...
            data_x.set_shape((self.img_size[0],self.img_size[1],3))
            image = tf.reshape(data_x, [self.dim_input])
